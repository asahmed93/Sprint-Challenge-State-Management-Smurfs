...
- [ ] What problem does the context API help solve? making state more managable, you know where it is plus you don't have to 'props drill'.
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application? actions are what make the user interactions happen, reducers check the actions and check which one it should utilize, store holds the state values and allows you to receive props from anywhere.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
application state is global and component state is state that is accessible locally. application I would use to manage states that need to be updated in several places where as component states are like form handles that need to be executed only locally.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

By default, redux dispatches synchronous actions, to communicate with an external API. Thunk returns a function instead of an action object, that function receives the store's dispatch method which is dispatched to regular synchronous action.

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
Honestly, I really don't know yet, I chose to do Redux on the sprint simply because I had the most and recent practice with Redux, if I was as comfortable with Context API I probably would've still chose redux because of Thunk and how it calls it's actions but again I need to review Context API again 
